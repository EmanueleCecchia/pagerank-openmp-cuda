...
Il calcolo di $\mathrm{contrib}[u]$ è separato dal ciclo di accumulo per una ragione di
...
elemento non nullo; qui al suo posto c'è \texttt{out\_deg[N]}. La
Figura~\ref{fig:csr}(b) mostra perché si può: tutti i non nulli della colonna
$u$ valgono $1/\mathrm{outdeg}(u)$, così che il valore dipenda dalla sola
colonna e sia ricavabile da \texttt{out\_deg}.
...
uscenti, e ognuna rifarebbe la stessa divisione. Il vettore \texttt{contrib}
dell'Algoritmo~\ref{alg:seq} la esegue invece una volta per nodo, prima
dell'accumulo; al ciclo che percorre i predecessori resta solo da leggere
\texttt{contrib[col\_idx[j]]} e sommarlo, senza divisioni e senza valori da
recuperare. Anche in memoria il guadagno viene dallo stesso passaggio da $M$
a $N$, con $N \ll M$: su \texttt{soc-LiveJournal1} un \texttt{val[M]} in
...
volte meno. Il solo \texttt{val[M]} peserebbe più dell'intera struttura CSR.
