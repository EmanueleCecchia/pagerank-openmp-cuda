...
memorizzano i soli valori non nulli di $\mathbf{MT}$, usando formati come
...
Il formato \textbf{CSR}, per salvare i valori non nulli della matrice
$\mathbf{MT}$, invece di utilizzare una matrice bidimensionale $N \times N$
usa tre array monodimensionali:
\begin{itemize}
  \item \texttt{col\_idx[M]}: gli indici di colonna dei non nulli, letti riga
    per riga; è la concatenazione delle liste di adiacenza, e la porzione
    relativa alla riga $v$ elenca i predecessori di $v$;
...
% ------------------------- (a) il grafo -------------------------
\node[tit] at (1.0,3.15) {(a) il grafo};
...
\begin{scope}[shift={(6.7,0)}]
  \node[tit] at (1.65,3.15) {(b) la matrice $\mathbf{MT}$};
  ...
  \node[idx] at (-0.55,2.62) {$u$};
  \node[idx,rotate=90] at (-0.6,{2.4-2.5*0.66}) {$v$};
...
\begin{scope}[shift={(0,-2.25)}]
  \node[tit,anchor=west] at (-2.1,0.95) {(c) i tre array};

  % --- row_ptr
  \node[nomearr] at (1.15,0) {row\_ptr};
  \foreach \k/\val in {0/0, 1/2, 2/4, 3/7, 4/8, 5/8} {
    \node[cella] (rp\k) at ({1.4+\k*0.85+0.425},0) {\val};
    \node[idx]   at ({1.4+\k*0.85+0.425},0.50) {\k};
  }

  % --- dove comincia ogni riga
  \foreach \k/\p in {0/0, 1/2, 2/4, 3/7, 4/8, 5/8} {
    \draw[->,thin,gray!75,dashed] (rp\k.south) -- ({1.4+\p*0.85},-1.03);
  }

  % --- col_idx
  \node[nomearr] at (1.15,-1.35) {col\_idx};
  \foreach \k/\val in {0/1, 1/4, 4/0, 5/1, 6/4} {
    \node[cella,fill=blue!13] at ({1.4+\k*0.85+0.425},-1.35) {\val};
  }
  \foreach \k/\val in {2/0, 3/2, 7/1} {
    \node[cella,fill=orange!20] at ({1.4+\k*0.85+0.425},-1.35) {\val};
  }
  \foreach \k in {0,...,7} {
    \node[idx] at ({1.4+\k*0.85+0.425},-1.86) {\k};
  }
  \foreach \x/\testo in {1/{riga 0}, 3/{riga 1}, 5.5/{riga 2}, 7.5/{riga 3}} {
    \node[nota] at ({1.4+\x*0.85},-2.28) {\testo};
  }
  \node[nota,anchor=west] at (8.35,-1.35) {riga 4: vuota};
...
\end{scope}
\end{tikzpicture}
\caption{Costruzione del CSR trasposto su un grafo con $N=5$ nodi e $M=8$
archi. In (b) la riga $v$ della matrice $\mathbf{MT}$ raccoglie i
...
\texttt{row\_ptr[v]} indica dove ciascuna comincia (le frecce tratteggiate) e
...
e \texttt{col\_idx[6]}, cioè i nodi $0$, $1$ e $4$ --- ed è esattamente ciò che
si legge nel grafo. Gli offset sono $N+1$ e non $N$ proprio per questo motivo:
la sentinella \texttt{row\_ptr[N]}~$=M$ fa sì che l'ultima riga si tratti come
tutte le altre.

\stacco
Due situazioni che di solito richiederebbero un caso particolare qui non ne
hanno bisogno, e la figura le contiene entrambe:
\begin{itemize}
  \item il nodo~4 non ha predecessori, quindi la sua riga è vuota:
    \texttt{row\_ptr[4] == row\_ptr[5]}. Il ciclo interno non viene eseguito
    nemmeno una volta e $\mathrm{PR}'(4)$ resta pari a $\mathit{base}$, senza
    alcun controllo aggiuntivo;
  \item il nodo~3 è \emph{dangling}, cioè privo di archi uscenti:
    \texttt{out\_deg[3] == 0}. È una condizione diversa dalla precedente, su un
    array diverso, e si gestisce altrove: il suo punteggio non va perso ma
    redistribuito (Sezione~\ref{sec:damping}).
\end{itemize}
