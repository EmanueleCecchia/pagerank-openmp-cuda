...
Un CSR generico ha un quarto array, \texttt{val[M]}, con il valore di ogni
elemento non nullo. Qui non serve, e la Figura~\ref{fig:csr}(b) mostra il
perché: tutti i non nulli della colonna $u$ valgono $1/\mathrm{outdeg}(u)$, così
che il valore dipenda dalla sola colonna e sia ricavabile da \texttt{out\_deg}.
Il vettore \texttt{contrib} dell'Algoritmo~\ref{alg:seq} porta l'osservazione
fino in fondo applicando quella divisione una volta per nodo: il ciclo interno
non contiene allora né divisioni né letture di valori, ma solo un'indirezione e
una somma. Il risparmio non è trascurabile --- su \texttt{soc-LiveJournal1} un
\texttt{val[M]} in doppia precisione occuperebbe $8M \approx 552$~MB, più di
tutto il resto della struttura messo insieme.
...
La struttura occupa $8(N{+}1) + 4M + 4N$ byte, ed è questa formula a produrre
l'ultima colonna della Tabella~\ref{tab:dataset}: per \texttt{web-Google}
$29{,}5$~MB contro i $5{,}6$~TB della matrice densa. Resta però un aspetto che
tornerà nella Sezione~\ref{sec:scalabilita}. \texttt{row\_ptr} e
...
\begin{itemize}
  ...
  \item \textbf{Reciproci del grado precalcolati.} Il ciclo dei contributi
    ...
    l'array costerebbe $8N$ byte contro i $4N$ di \texttt{out\_deg},
    raddoppiando il traffico proprio dove sta il collo di bottiglia. Vale
    invece la pena provarlo sulla GPU e nella variante \texttt{float}, dove
    il rapporto di costo fra divisione e moltiplicazione è più sfavorevole e
    l'occupazione resta invariata. Da verificare l'effetto sulla precisione:
    $x \cdot (1/d)$ introduce un arrotondamento in più rispetto a $x/d$.
