...
parallele, confrontando una versione puramente multicore basata su OpenMP
con una versione ibrida che distribuisce il carico fra CPU e GPU tramite
CUDA. Il grafo è memorizzato in formato CSR trasposto, scelta che rende il
...
Formalmente, dato un grafo orientato $G=(V,E)$ con $N=|V|$, il vettore dei
rank $\pi$ è il punto fisso dell'iterazione
\begin{equation}
\pi_{t+1}(v) \;=\; \frac{1-d}{N}
\;+\; d\left( \sum_{u \,:\, u \to v} \frac{\pi_t(u)}{\mathrm{outdeg}(u)}
\;+\; \frac{D_t}{N} \right)
\label{eq:pagerank}
\end{equation}
dove $d$ è il \emph{damping factor} (convenzionalmente $0{,}85$) e
$D_t=\sum_{u\,:\,\mathrm{outdeg}(u)=0}\pi_t(u)$ è la massa di rank che si
trova sui nodi \emph{dangling}, cioè privi di archi uscenti.

\subsection{Il termine dangling}
Il termine $D_t/N$ non è un dettaglio implementativo ma una necessità
...
La conseguenza pratica è che $\sum_v \pi(v) = 1$ costituisce un test di
...
\label{alg:seq}
\begin{algorithmic}[1]
\State $\pi(v) \gets 1/N$ \textbf{per ogni} $v \in V$
\Repeat
  \State $D \gets 0$
  \ForAll{$u \in V$}
    \If{$\mathrm{outdeg}(u) = 0$}
      \State $c(u) \gets 0$; \quad $D \gets D + \pi(u)$
    \Else
      \State $c(u) \gets \pi(u)/\mathrm{outdeg}(u)$
    \EndIf
  \EndFor
  \State $\mathit{base} \gets (1-d)/N + d\cdot D/N$
  \ForAll{$v \in V$}
    \State $\pi'(v) \gets \mathit{base} + d \sum_{u \to v} c(u)$
  \EndFor
  \State $\varepsilon \gets \sum_v |\pi'(v)-\pi(v)|$
  \State scambia $\pi$ e $\pi'$
...
$\pi_{t+1}(v)$, l'insieme dei nodi che puntano a $v$. Esistono due modi di
organizzare il calcolo:

\begin{description}
  \item[Push] si scorre l'insieme degli archi $u \to v$ e si somma il
    contributo di $u$ dentro $\pi'(v)$. Più archi convergono sullo stesso
  ...
  \item[Pull] si scorre l'insieme dei nodi $v$, si leggono i suoi
    predecessori e si scrive $\pi'(v)$ una sola volta. Ogni nodo è di
