...
\texttt{soc-LiveJournal1} un \texttt{val[M]} in
doppia precisione occuperebbe $8 \times M$ byte, cioè circa $552$~MB, mentre
a \texttt{out\_deg} bastano $4 \times N$ byte, circa $19$~MB: quasi trenta
volte meno.
...
per \texttt{web-Google} $29{,}5$~MB contro i $5{,}6$~TB della matrice densa.
...
operazioni aritmetiche è bassissimo, ed è la ragione di fondo per cui il
calcolo si rivelerà \emph{memory-bound}.
...
possibile solo sul grafo più piccolo: 386~MB per \texttt{wiki-Vote} contro
