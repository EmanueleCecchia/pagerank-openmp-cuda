...
La definizione è circolare e si scioglie iterativamente: si parte da una
...
rank $\mathrm{PR}$ è il punto fisso dell'iterazione
\begin{equation}
...
\begin{tabular}{@{}ll@{}}
\toprule
$\mathrm{PR}_t(v)$ & punteggio del nodo $v$ alla $t$-esima iterazione \\
...
I tre addendi sono i tre modi in cui un nodo riceve punteggio: una quota
uguale per tutti (il salto casuale del navigatore), il contributo dei
predecessori, e la redistribuzione della \emph{dangling mass} discussa qui
sotto. Il damping factor governa anche la convergenza: con $d<1$
l'iterazione è una contrazione, quindi il punto fisso esiste ed è unico.
